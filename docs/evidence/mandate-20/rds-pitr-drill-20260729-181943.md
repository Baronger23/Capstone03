...
Video links: pending Drive URLs
```
...
|---|---|---|---|
| 1 | Create GOOD marker and establish pre-corruption restore point | captured | `<pending-drive-link>` |
| 2 | Controlled corruption after `LatestRestorableTime >= T_restore` | captured | `<pending-drive-link>` |
| 3 | Restore request / RTO completion | captured partially; terminal context issue occurred after RTO | `<pending-drive-link>` |
| 4 | Correct drill endpoint verification on separate port/session | captured | `<pending-drive-link>` |
...
Evidence links: pending Drive URLs
