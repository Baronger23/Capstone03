...
# --- In-Memory Cache (Phase 1 / Tuần 1) ---
_summary_cache = {}  # product_id -> (summary_string, cached_at)
CACHE_TTL_SECONDS = 3600  # 1 giờ

def get_cached_summary(product_id: str):
    if product_id in _summary_cache:
        summary, cached_at = _summary_cache[product_id]
        if time.time() - cached_at < CACHE_TTL_SECONDS:
            return summary
    return None

def set_cached_summary(product_id: str, summary: str):
    _summary_cache[product_id] = (summary, time.time())
...
def get_ai_assistant_response(request_product_id, question):
    ...
    with tracer.start_as_current_span("get_ai_assistant_response") as span:
        ...
        # 1. Check In-Memory Cache (P3)
        cached_res = get_cached_summary(request_product_id)
        if cached_res:
            logger.info(f"Cache HIT for product_id: {request_product_id}")
            span.set_attribute("app.ai_assistant.cache_hit", True)
            ai_assistant_response.response = cached_res
            product_review_svc_metrics["app_ai_assistant_counter"].add(1, {'product.id': request_product_id, 'cache': 'hit'})
            return ai_assistant_response

        span.set_attribute("app.ai_assistant.cache_hit", False)
        ...
        if llm_rate_limit_error:
            ...
            if random_number < 0.5:
                ...
                try:
                    ...
                    stale_summary = get_cached_summary(request_product_id)
                    if stale_summary:
                        ai_assistant_response.response = stale_summary
                    else:
                        ai_assistant_response.response = "Tóm tắt tạm thời không khả dụng. Vui lòng tham khảo các đánh giá chi tiết bên dưới."
                    product_review_svc_metrics["app_ai_assistant_counter"].add(1, {'product.id': request_product_id, 'status': 'error'})
                    return ai_assistant_response
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.error(f"All LLM retries exhausted: {e}")
                    span.record_exception(e)
                    span.set_status(Status(StatusCode.ERROR, description=str(e)))
                    stale_summary = get_cached_summary(request_product_id)
                    if stale_summary:
        ...
        if tool_calls:
            ...
            try:
                ...
                # BUG 3 FIX: Do NOT cache if llmInaccurateResponse is active
                if not llm_inaccurate_response:
                    set_cached_summary(request_product_id, result)
                else:
                    logger.info(f"Skipping cache because llmInaccurateResponse is active for {request_product_id}")
            except Exception as e:
                logger.error(f"Final LLM synthesis failed: {e}")
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, description=str(e)))
                stale_summary = get_cached_summary(request_product_id)
                if stale_summary:
            ...
            res_text = response_message.content if response_message.content else "Không có tóm tắt."
            ai_assistant_response.response = res_text
            if not llm_inaccurate_response:
                set_cached_summary(request_product_id, res_text)
