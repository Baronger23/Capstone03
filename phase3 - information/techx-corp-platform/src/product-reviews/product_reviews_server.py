...
try:
    ...
    # NOT_SERVING/UNIMPLEMENTED are required by ProductReviewService.Check()/Watch()
    # (REL-02 dependency-aware readiness + graceful shutdown). The upstream fallback
    # only defined SERVING, which would AttributeError on the unhealthy path.
    health_pb2 = SimpleNamespace(
...
# touching the two constants below. (Re-applied on top of the AIO02 upgrade: the
# upstream AIO02 branch does not carry this fix, and dropping it would regress
# PM-0016, which shipped to production on 28/07/2026 via PR #531.)
#
# GetProductReviews/GetAverageProductReviewScore and AskProductAIAssistant share
# ONE ThreadPoolExecutor (see GRPC_MAX_WORKERS in __main__), and that executor
...
# that worker before shedding, which meaningfully helps under a moderate AI burst.
# It does NOT reorder the shared queue, so it cannot stop a fast read RPC from
# queueing behind however many AI calls were already submitted ahead of it.
#
...
#   N=10  -> ~34ms   ·  N=30 -> ~178ms  ·  N=100 -> ~758ms (EXCEEDS the frontend's
#   500ms PRODUCT_REVIEWS_DEADLINE_MS)
# The admission wait MUST stay short: a request that fails to acquire still blocks
# its own grpc worker for up to that long before shedding. The durable fix is a
# separate thread pool / deployment for AskProductAIAssistant (postmortem P3, not
# shipped yet). The circuit breaker added by the AIO02 upgrade is complementary,
# not a replacement: it trips on Bedrock *failures*, whereas this caps *concurrency*
# even while Bedrock is succeeding slowly.
...
class ProductReviewService(demo_pb2_grpc.ProductReviewServiceServicer):
    ...
    def AskProductAIAssistant(self, request, context):
        ...
        # PM-0016 admission control (see AI_ASSISTANT_MAX_CONCURRENCY above).
    ...
    def Check(self, request, context):
        # REL-02 (dependency-aware readiness) — re-applied on top of the AIO02
        # upgrade, which had reduced this to an unconditional SERVING. This repo
        # deliberately made product-reviews readiness DB-aware so a pod that
        # cannot reach Postgres/RDS is pulled from the Service endpoints instead
        # of accepting traffic it will only fail. The readiness probe in
        # values-prod.yaml is a gRPC health check against this method.
        #
        # Also honours the AIO02 graceful-shutdown path: once SIGTERM has been
        # received we report NOT_SERVING immediately so Kubernetes drains this
        # pod before the gRPC server stops accepting work.
...
if __name__ == "__main__":
    ...
    # REL-02: register OUR servicer (not grpc_health's static HealthServicer) so
    # Check() runs the dependency-aware probe on every call. The static servicer
    # would always answer SERVING regardless of DB health. Graceful shutdown is
    # preserved via shutdown_event, which Check() honours (see the method).
    ...
    # shutdown_event is already set by the signal handler; ProductReviewService.Check()
    # reads it and answers NOT_SERVING from here on (REL-02 servicer, see __main__).
    logger.info("Reporting gRPC Health status NOT_SERVING (shutdown_event set)...")
