...
```
                       [ AI Engine đề xuất Action ]
                                     │
                                     ▼
                     [ Giai đoạn 5.1: Policy & Safety ]
                     ├─► Dry-run Check (Cú pháp, phân quyền)
                     └─► Blast Radius (Tác động service, % traffic, risk)
                                     │
                                     ▼
                     [ Giai đoạn 5.2: Risk Assessment ]
                     ┌───────────────┼───────────────┐
                     │               │               │
                 [ LOW ]         [ MEDIUM ]      [ HIGH ]
              (Rủi ro thấp)    (Rủi ro trung bình)  (Rủi ro cao/Lệnh lạ)
              Vd: cache-flush, Vd: scale replica,  Vd: lệnh ngoài whitelist,
              breaker-force    restart pod         đụng config flagd BTC
                     │               │               │
                     ▼               ▼               ▼
                 [ Execute ]    [ Gửi Button ]   [ Reject ]
               (Chạy tự động)   (Chờ duyệt)    (Hủy & Cảnh báo)
                     │               │
                     └───────┬───────┘
                             │ (Thực thi lệnh K8s API)
                             ▼
                 [ Giai đoạn 5.3: Verify & Rollback ]
                 (Chạy Timer đếm ngược - Tối đa 5 phút)
                             │
                             ▼
                 [ Quét Telemetry kiểm tra 5 phút ]
                 (Verify metrics: Latency, Error Rate)
                 ┌───────────┴───────────┐
       (Thành công / No Timeout)         │ (Thất bại / Gặp Timeout)
                 ▼                       ▼
        [ Hoàn thành & Log ]     [ 1. Chạy Rollback Plan ]
                                 [ 2. Escalate báo SRE người ]
```

* **Chi tiết Phân loại Rủi ro (Risk Assessment)**:
  - **Mức THẤP (Low Risk - Tự động thực thi)**: Áp dụng cho các hành động an toàn, không gây gián đoạn hệ thống (ví dụ: `cache-flush` xóa cache, hoặc `breaker-force` kích hoạt breaker). AI Engine tự động chạy lệnh và ghi log.
  - **Mức TRUNG BÌNH (Medium Risk - Cần duyệt)**: Áp dụng cho các hành động thay đổi tài nguyên vật lý trong K8s (ví dụ: `scale` deployment, `restart` pod). Bắt buộc phải gửi button card lên Slack/Discord chờ người trực click **Approve** trong 5 phút.
  - **Mức CAO (High Risk - Tự động chặn/Reject)**: Áp dụng cho các đề xuất không nằm trong whitelist của **C6** hoặc có hành vi đụng chạm tới cấu hình `flagd` của BTC (vi phạm luật chơi). Engine sẽ tự động chặn đứng, hủy bỏ thực thi và phát cảnh báo critical về hành vi xâm phạm an toàn.

* **Quy tắc Bất biến C6 (Invariants)**:
  - **Timeout Control**: Mỗi lệnh thực thi (K8s API call) được gắn timer tối đa **5 phút**. Quá 5 phút $\rightarrow$ Tự động hủy lệnh, kích hoạt Rollback Plan và ghi log trạng thái `result: timeout` (Invariant 3).
  - **Rate Limit**: Engine tự động đếm số lần chạy hành động cho mỗi incident. Nếu vượt quá **3 actions/incident/giờ** $\rightarrow$ Engine tự động khóa, ngừng tự động thực thi và chuyển sang báo động người trực (Invariant 4).
  ...
  - **Rollback thất bại $\rightarrow$ Escalate**: Nếu Rollback Plan chạy mà vẫn thất bại (K8s API lỗi, timeout, hoặc telemetry vẫn xấu sau 5 phút) $\rightarrow$ Engine chuyển về **Manual Mode** và gửi alert khẩn cấp tới kỹ sư on-call kèm đầy đủ context: `incident_id`, lệnh đã thực thi, kết quả rollback, và link Evidence Pack. Người kỹ sư toàn quyền xử lý từ đây.
